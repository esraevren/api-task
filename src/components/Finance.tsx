...
function Finance() {
  ...
  const getFinance = async () => {
    ...
    console.log(response.data.mainEntity.itemListElement);
    setFinance({
      dolar:
        response.data.mainEntity.itemListElement[0].currentExchangeRate.price,
      euro: response.data.mainEntity.itemListElement[1].currentExchangeRate
        .price,
      altın:
  ...
  return (
    <div className="border border-r-gray-400 w-40 h-screen">
      <div className="flex border shadow-2xl pt-5 pb-5 w-36 mb-3 mt-5 mr-3">
        <img src={dolar} alt="dolar" className="w-6 h-6 ml-1" />
      ...
      <div className="flex border  shadow-2xl pt-5 pb-5 w-36 mb-3">
        <img src={euro} alt="euro" className="w-6 h-6 ml-1" />
